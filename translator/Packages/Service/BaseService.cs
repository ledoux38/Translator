...
public class BaseService
{
    ...
    public static void SortAllJsonFiles(IConfiguration configuration)
        ...
        public static void OrganizeAllJsonFiles(IConfiguration configuration)
        {
            var basePath = PathResolver.ResolvePath(configuration["BasePath"] ?? Directory.GetCurrentDirectory());
            var excludedFiles = configuration["ExcludedFiles"]?.Split(',') ?? Array.Empty<string>();
            var jsonFiles = FileManager.GetFilesExcludingNodeModules(basePath, "*.json", excludedFiles.ToList());

            foreach (var file in jsonFiles)
            {
                try
                {
                    var jsonToken = FileManager.LoadJson(file);
                    var organizedJson = JsonOrganizer.OrganizeJson(jsonToken);

                    if (organizedJson is JObject jsonObject)
                    {
                        FileManager.SaveJson(file, jsonObject);
                        Console.WriteLine($"Réorganisation effectuée pour le fichier : {file}");
                    }
                    else
                    {
                        Console.WriteLine($"Le fichier {file} ne contient pas un objet JSON racine valide.");
                    }
                }
                catch (Exception ex)
                {
